...
#import "CutPhotoViewController.h"
...
@interface PhotosViewController () <UITableViewDataSource,UITableViewDelegate,CutPhotoViewControllerDelegate>
...
@property (nonatomic,strong) CardSwitchView *cardSwitchView;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString * cellIdentifier = @"PhotoFirstCollectionViewCell";
    if (indexPath.row > 0) {
        cellIdentifier = @"PhotoSecondCollectionViewCell";
    }
    ...
    if (indexPath.row > 0) {
        ...
        NSString *str = [NSString stringWithFormat:@"%ld",(long)[self isExistence:asset]];
...
#pragma mark CardSwitchViewDelegate 小图查看 代理

- (void)cardSwitchViewDidScroll:(CardSwitchView *)cardSwitchView index:(NSInteger)index {
...
- (void)setRightBarButtonItems:(NSString *)redDotValue {
    ...
    UIBarButtonItem *item = [self itemRightItemWithTitle:@"下一步" selector:@selector(nextButtonAction:)];
    [ary addObject:item];
    
    if (redDotValue.intValue > 0) {
        item = [[UIBarButtonItem alloc] initWithCustomView:[self itemRedViewWithRedDotValue:redDotValue]];
        [ary addObject:item];
    }
...
- (void)gotoCutPhotoViewController {
    ...
    CutPhotoViewController *imgCropperViewController = [[CutPhotoViewController alloc] initWithImage:image  cropSize:cropSize title:title isLast:isLast];
...
- (UITableView *)dropDownTableView {
    if (!_dropDownTableView) {
        ...
        [_dropDownTableView registerNib:[UINib nibWithNibName:@"PhotoCell" bundle:nil] forCellReuseIdentifier:@"PhotoCell"];
...
- (CardSwitchView *)cardSwitchView {
    if (!_cardSwitchView) {
        _cardSwitchView = [[CardSwitchView alloc] initWithFrame:CGRectMake(0,0, self.view.frame.size.width, CardSwitchViewHeight)];
...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        [self.collectionView registerNib:[UINib nibWithNibName:@"PhotoFirstCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"PhotoFirstCollectionViewCell"];
        [self.collectionView registerNib:[UINib nibWithNibName:@"PhotoSecondCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"PhotoSecondCollectionViewCell"];
