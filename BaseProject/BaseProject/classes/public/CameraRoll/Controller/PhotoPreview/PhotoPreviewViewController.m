...
@interface PhotoPreviewViewController () <UIScrollViewDelegate,UIActionSheetDelegate>
...
- (void)showPhotos {
    ...
    for (PhotoZoomingScrollView *photoView in _visiblePhotoViews) {
...
- (void)showPhotoViewAtIndex:(NSInteger)index {
    PhotoZoomingScrollView *photoView = [self dequeueReusablePhotoView];
    if (photoView == nil) {
        photoView = [[PhotoZoomingScrollView alloc] init];
    }
...
//获取可重用的view
- (PhotoZoomingScrollView *)dequeueReusablePhotoView {
    PhotoZoomingScrollView *photoView = [_reusablePhotoViews anyObject];
    if (photoView) {
